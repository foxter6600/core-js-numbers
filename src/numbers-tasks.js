...
function roundToPowerOfTen(num, pow) {
  const reminder = num % 10 ** pow;
  const a = Math.floor(reminder / (5 * 10 ** (pow - 1)));
  return a === 1 ? num + (10 ** pow - reminder) : num - reminder;
}
...
function getCountOfOddNumbers(n) {
  let res = 0;
  for (let num = Math.abs(n); num >= 1; num -= 1) {
    if (num % 2 !== 0) {
      res += 1;
    }
  }
